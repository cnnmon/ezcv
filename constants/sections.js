import { trigger, COLORS } from "./index";
import { getMultiSectionDefaultFieldsString, getBasicFormat } from "./utils";
import { RiMoonClearFill, RiBriefcase2Fill, RiRainbowFill, RiGroupFill, RiStarSmileFill, RiAwardFill, RiToolsFill } from "react-icons/ri";
...
const SECTIONS = [
  {
    ...
    body: [
      {
        other: ["mailto:firstlast@gmail.com", "yourwebsite.com", "(111)-222-3333"],
      },
    ],
    ...
    body: [
      {
        ...
        other: ["GPA: 0.0/4.0", "Relevant Coursework: Irish Literature, Celtic Mythology, Comparative and Historical Linguistics, Scandinavian Folklore"],
      },
    ],
    ...
    body: [
      {
        ...
        other: ["- Clown Infra Team", "- Optimized happiness efficiency by 99%"],
      },
      {
        ...
        description: "Toronto",
        other: ["- Designed delightful experiences for children", "- Lead a team organizing balloons into animals"],
      },
    ],
  ...
  },
  {
    ...
    body: [
      {
        other: ["Code: JavaScript, Python, Java, C#, C", "Frameworks/Tools: React, NodeJS, Unity, Git", "Design: Figma, Photoshop, InDesign"],
      },
    ],
    icon: RiToolsFill,
    color: COLORS.purple,
...
function getSectionsFormat({
  ...
  const char = `${trigger}${type} ${name}\n\n${getMultiSectionDefaultFieldsString(body)}`;
  return ({
...
export const getSections = () => SECTIONS.map((s, index) => getSectionsFormat(s, index));
